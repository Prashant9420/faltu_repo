...
function DocumentPage() {
	...
	const handleSaveDocument = async () => {
		try {
			const docxBlob = await documentEditorRef.current.documentEditor.saveAsBlob('Docx');
			const sfdtBlob=await documentEditorRef.current.documentEditor.saveAsBlob('Sfdt')
			console.log(await blobToText(sfdtBlob))
	...
	useEffect(() => {
		const fetchData = async () => {
			const docxFile = await urltoFile(file.data, 'test.docx', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
